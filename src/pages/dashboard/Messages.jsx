...
import { MessageSquare, Bell, Users, Wifi, WifiOff, Search, RefreshCw } from 'lucide-react';
...
const MessagesInner = () => {
    ...
    // Role-aware: sellers use /conversations/seller, buyers use /conversations/user
    ...
    const loadConversations = useCallback(async (silent = false) => {
        ...
        try {
            const res = isSeller
                ? await chatService.getSellerConversations()
    ...
    // Refresh list when socket fires conversation_updated
    useEffect(() => {
        if (!socket) return;
    ...
    const handleSelect = (conv) => {
        ...
        setConversations(prev => prev.map(c =>
            c._id === conv._id
                ? { ...c, unreadByUser: 0, unreadBySeller: 0 }
                : c
        ));
    };
    ...
    // Recipient info depends on which side you're on
    const recipientName = selectedConv
        ? isSeller
            ? `${selectedConv.userId?.firstName || ''} ${selectedConv.userId?.lastName || ''}`.trim() || 'Customer'
            : selectedConv.sellerId?.storeName || 'Seller'
        : '';
    const recipientAvatar = selectedConv
        ? isSeller
            ? selectedConv.userId?.avatar?.url
            : selectedConv.sellerId?.logo?.url
        : null;
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">
                                        {isSeller ? 'Respond to your customers' : 'Chat with sellers about products and orders'}
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="flex h-[calc(100vh-180px)] bg-gray-50">
                    ...
                    <div className="flex-1 flex flex-col">
                        <ChatWindow
                            ...
                            currentRole={isSeller ? 'seller' : 'user'}
                            recipientName={recipientName}
                            recipientAvatar={recipientAvatar}
                            subject={selectedConv?.subject}
                            recipientUserId={selectedConv?.sellerId?.userId?._id}
                        />
                    </div>
                </div>
            </div>
